...
| `npm run test:migrations` | **Real PostgreSQL** integration tests; **fails** if `psql`/PostgreSQL unavailable |
...
2. Apply through Supabase migration tooling so history is recorded:
   ```bash
   supabase db push --linked
   ```
   Or paste into Supabase SQL editor **only if** you also register the version (Option B).
...
Or insert into the migration history table on hosted Supabase:

```sql
INSERT INTO supabase_migrations.schema_migrations (version)
VALUES ('20260825155900')
